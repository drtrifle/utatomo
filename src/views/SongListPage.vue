    ...
    >
      <template v-slot:item.title="{ item }">
        <div class="d-flex align-center py-2">
          ...
          ></v-img>
          <div>
            <div class="font-weight-bold">{{ item.raw.title }}</div>
            <div class="text-caption">{{ item.raw.artist }}</div>
          </div>
        </div>
      </template>
...
const goToSongDetails = (_event: Event, { item }: { item: any }) => {
  const song = item.raw as SongInfo;
